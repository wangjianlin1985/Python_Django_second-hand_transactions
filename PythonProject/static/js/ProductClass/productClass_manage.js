...
function initProductClassManageTool() {
	productClass_manage_tool = {
		...
		search: function() {
			$("#productClass_manage").datagrid("options").queryParams=queryParams; 
			$("#productClass_manage").datagrid("load");
		},
		exportExcel: function() {
			$("#productClassQueryForm").form({
			    url:"/ProductClass/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/ProductClass/update/" + rows[0].classId,
					type : "get",
					data : {
