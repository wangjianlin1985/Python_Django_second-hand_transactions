...
function initAreaInfoManageTool() {
	areaInfo_manage_tool = {
		...
		search: function() {
			$("#areaInfo_manage").datagrid("options").queryParams=queryParams; 
			$("#areaInfo_manage").datagrid("load");
		},
		exportExcel: function() {
			$("#areaInfoQueryForm").form({
			    url:"/AreaInfo/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/AreaInfo/update/" + rows[0].areaId,
					type : "get",
					data : {
