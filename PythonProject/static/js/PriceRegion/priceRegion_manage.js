...
function initPriceRegionManageTool() {
	priceRegion_manage_tool = {
		...
		search: function() {
			$("#priceRegion_manage").datagrid("options").queryParams=queryParams; 
			$("#priceRegion_manage").datagrid("load");
		},
		exportExcel: function() {
			$("#priceRegionQueryForm").form({
			    url:"/PriceRegion/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/PriceRegion/update/" + rows[0].regionId,
					type : "get",
					data : {
